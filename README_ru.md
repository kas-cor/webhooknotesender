...
- Топ-5 самых используемых профилей (по частоте использования) доступны в меню долгого нажатия
- Обновляются автоматически при запуске, после захватов, сохранений и удалений
- Отдельный префикс `"app_shortcut_"` исключает коллизию с закреплёнными ярлыками
...
| [Unreleased] | — | Ярлыки приложения (long-press), счётчик use_count, удалён сломанный транскодинг видео, фиксы жизненного цикла ярлыков |
